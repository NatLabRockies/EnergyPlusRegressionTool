...
class RegressionManager:
    ...
    def __init__(self, mute: bool = False):
        ...
        self.threshold_file: Path = Path(__file__).parent / 'diffs' / 'math_diff.config'
        self.mute = mute
...
<html>
 ...
 <body>
  ...
  <script>hljs.highlightAll();</script>
    ...
    @staticmethod
    def build_row_in_single_test_case_html(diff_file_name: str) -> str:
        return f"""
   <tr>
    ...
    @staticmethod
    def build_single_test_case_html(contents: str) -> str:
        return f"""
<!doctype html>
<html>
 <head>
        ...
        if num_diff_types > 0:
            for k in diff_type_keys:
                ...
                for idf in diffs_this_type:
                    cl = "list-group-item list-group-item-action"
                    this_diff_type_list += f"""<a href="{idf}/index.html" class="{cl}">{idf}</a>\n"""
                diff_type_content += f"""
...
{this_diff_type_list}
        ...
        for idf in sorted_idf_keys:
            ...
            col_1 = f"<td><a href='{idf}/index.html'>{idf}</a></td>"
            col_2 = f"<td>{base_time}</td><td>{branch_time}</td>"
            run_time_rows_text += f"""<tr>{col_1}{col_2}</tr>"""
        s_1 = f"{sum_base_seconds:.1f}"
        s_2 = f"{sum_branch_seconds:.1f}"
        run_time_rows_text += f"""<tr><td>Runtime Total (Successes)</td><td>{s_1}</td><td>{s_2}</td></tr>"""
...
{diff_lines}
    ...
    def generate_regression_plotter(bundle_root: Path, metadata_object: dict, results_object: dict, limit: bool):
        ...
        if limit:
            limit_string = r"""
            <div id="size-alert" class="alert alert-warning" role="alert">
                The regressions hit a size limit and were truncated, not all files may be represented here.
                Double check the full regression package.
                </div>
            """
    ...
    @staticmethod
    def get_diffs_for_single_entry(entry: TestEntry) -> list[str]:
        this_file_diffs = []
        diffs_this_entry = []
        text_diff_results = {
            "Audit": entry.aud_diffs,
        ...
        for diff_type, diffs in text_diff_results.items():
            ...
            if diffs.diff_type != TextDifferences.EQUAL:
                this_file_diffs.append(diff_type)
                diffs_this_entry.append(diff_type)
        ...
        for diff_type, diffs in numeric_diff_results.items():
            ...
            if diffs.diff_type == 'Big Diffs':
                this_file_diffs.append(f"{diff_type} Big Diffs")
                diffs_this_entry.append(f"{diff_type} Big Diffs")
            elif diffs.diff_type == 'Small Diffs':
                this_file_diffs.append(f"{diff_type} Small Diffs")
                diffs_this_entry.append(f"{diff_type} Small Diffs")

        if entry.table_diffs:
            if entry.table_diffs.big_diff_count > 0:
                this_file_diffs.append("Table Big Diffs")
                diffs_this_entry.append("Table Big Diffs")
            elif entry.table_diffs.small_diff_count > 0:
                this_file_diffs.append("Table Small Diffs")
                diffs_this_entry.append("Table Small Diffs")
            if entry.table_diffs.string_diff_count > 1:  # There's always one...the time stamp
                this_file_diffs.append("Table String Diffs")
                diffs_this_entry.append("Table String Diffs")

        return diffs_this_entry

    def build_summary_content(self, completed_structure: CompletedStructure):
        ...
        results_object = {}
        hit_max_limit = False
        base_dir = completed_structure.results_dir_a
        mod_dir = completed_structure.results_dir_b
        for entry in completed_structure.entries_by_file:
            if entry.eso_diffs and entry.eso_diffs.diff_type != 'All Equal' and not hit_max_limit:
                self.root_index_files_diffs.append(entry.basename)
                # get data from the csvs, for now just all columns
                basename = entry.basename
                base_csv = base_dir / basename / "eplusout.csv"
                base_column_data = self.read_csv_to_columns(base_csv)
                base_timestamps = base_column_data['Date/Time']
                mod_csv = mod_dir / basename / "eplusout.csv"
                ...
                csv_diff_file = base_dir / basename / "eplusout.csv.absdiff.csv"
                abs_diff_columns = self.read_csv_to_columns(csv_diff_file)
